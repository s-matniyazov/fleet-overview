...
import {showMessage, TIME_ZONES} from "@/util/utils.js";
...
function getData() {
  axiosIns.get(`${apiUrl}/with-files`)
      .then(res => {
      ...
      <template #row_hvut_2290="{row}">
        <td>
          <FileMiniCard name="HVUT_2290" type="HVUT_2290"
                        ...
                        @click="(e) => {selectedRow = row; selectFileSection('HVUT_2290'); e.stopPropagation()}"/>
        </td>
      </template>
      ...
      <template #row_w_9="{row}">
        <td>
          <FileMiniCard name="W_9" type="W_9"
                        ...
                        @click="(e) => {selectedRow = row; selectFileSection('W_9'); e.stopPropagation()}"/>
        </td>
      </template>
