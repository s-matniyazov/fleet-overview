...
const apiUrl = URIS.TASK_PRIORITIES;
const dataList = ref([]);
        ...
        <UDialog :show="addModal" @close="addModal = false" @save="onSave">
          <template #header>
            <div class="d-flex" style="width: 100%">
              <div class="text-dark">
                {{ data.id ? t('edit') : t('add') }} {{t('task_priority')}}
              </div>
          ...
          <template #body>
            <form class="needs-validation" novalidate="">
              <div class="row">
                ...
                <!--            description-->
                <div class="col-12">
                  <UTextarea v-model="data.description" :label="t('describe')" :placeholder="t('enter_priority_description')"
                             classes="mb-3"/>
                </div>
              </div>
            </form>
