...
const apiUrl = URIS.TEAM_STATUSES;
const dataList = ref([]);
    ...
    <div class="row align-items-center">
      <div class="col-12">
        <div class="d-flex flex-wrap align-items-center justify-content-start gap-2 mb-2">
          <div class="d-flex">
            <button @click="showMessage('something')" class="btn btn-primary btn-sm mx-1"><span class="mdi mdi-plus"></span> {{ t('add') }}</button>
      ...
      <template #header>
        <div class="d-flex">
          <div class="d-flex" style="width: 100%">
            {{ data.id ? t('edit') : t('add') }} {{ t('team_status') }}
          </div>
      ...
      <template #body>
        <form class="needs-validation" name="event-form" novalidate="">
          <div class="row">
            <!--            name-->
            <div class="col-12">
              <UInput v-model="data.name" :label="t('name')" :hint="t('name')" :name="t('name')" :placeholder="t('enter_status_name')" classes="mb-3" />
            </div>

            <!--            description-->
            <div class="col-12">
              <UTextarea v-model="data.description" :label="t('description')" :placeholder="t('enter_status_description')" classes="mb-3" />
            </div>
          </div>
        </form>
      </template>
