...
const cityCountryItems = computed(() =>
  mapSelectItems(countries.value, "name", "id"),
);
const { errorMessages: cityCountryErr } = useFormSelectField(
  "country",
...
<template>
  ...
  <Teleport to="body">
    <modal :show="addModal" @close="addModal = false">
      ...
      <template #body>
        <UForm id="ref-city-form" @submit="onSave">
          <div class="row">
            <!--            name-->
            <div class="col-12">
      ...
      <template #actions>
        <v-btn type="submit" form="ref-city-form" color="primary">Save</v-btn>
      </template>
    </modal>
  </Teleport>
</template>
