...
const fileOverlayCountryItems = computed(() =>
  mapSelectItems(stateStore.countries, "name", "id"),
);
const fileOverlayClassItems = computed(() =>
  mapSelectItems(DRIVER_FILE_CLASSES, "value", "key"),
);
const fileOverlayPeriodItems = computed(() =>
  mapSelectItems(filePeriods(), "name", "name"),
);

function fileOverlayStateItems(countryId) {
  return mapSelectItems(stateStore.getStates(countryId), "name", "id");
}
        ...
        <UForm @submit="saveFile(index)">
          <div class="card-body row">
            ...
            <template v-if="['CDL'].includes(data.type)">
              ...
              <div class="col-6">
                <v-select
                  v-model="upload.data.countryId"
                  :items="fileOverlayCountryItems"
                  item-title="title"
                  item-value="value"
                  :label="t('country')"
                  variant="outlined"
              ...
              <div class="col-6">
                <v-select
                  v-model="upload.data.stateId"
                  :items="fileOverlayStateItems(upload.data.countryId)"
                  item-title="title"
                  item-value="value"
                  :label="t('state')"
                  variant="outlined"
              ...
              <div class="col-6">
                <v-select
                  v-model="upload.data.classType"
                  :items="fileOverlayClassItems"
                  item-title="title"
                  item-value="value"
                  :label="t('class')"
                  variant="outlined"
            ...
            <template v-else-if="['HVUT_2290'].includes(data.type)">
              <div class="col-6">
                <v-select
                  v-model="upload.data.filedPeriod"
                  :items="fileOverlayPeriodItems"
                  item-title="title"
                  item-value="value"
                  :label="t('filedPeriod')"
                  variant="outlined"
